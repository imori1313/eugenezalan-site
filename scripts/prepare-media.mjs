...
await run(['-i','public/videos/hero-mobile-original.mp4','-map','0:v:0','-map','0:a:0','-vf','scale=720:1280,setsar=1','-c:v','libx264','-preset','slow','-crf','23','-maxrate','2400k','-bufsize','4800k','-pix_fmt','yuv420p','-c:a','aac','-b:a','96k','-movflags','+faststart','public/videos/hero-mobile.mp4']);
...
const sizes={};for(const name of ['hero-desktop.mp4','hero-mobile.mp4','craft-web.mp4'])sizes[name]=(await fs.stat(`public/videos/${name}`)).size;
